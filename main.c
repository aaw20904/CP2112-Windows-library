...
//Author: Andrii Androsovych. The code was written in Code::Blocks
 HID_SMBUS_STATUS operationResult;  //status of an operatoin
      ...
      WORD returned = 0;
      unsigned char someCharVar;
      unsigned char bytesRead;
      FILE* dbgFile;
      unsigned short numberOfBytes;

      HID_SMBUS_S0  	statusRx;
		HID_SMBUS_S1  	detailedStatusRx;
		WORD   	numRetriesRx;
		WORD   	bytesReadRx;
    char informationMessage[512]={0};
     unsigned short bigConstants[1024] = {
    ...
    //char infoMessages[256];


int main() {
   ///1) Load DLL file
    if( loadDynamicLibrary(&adapterHandle,informationMessage) != 0) {
        puts("Error dll library initialization! \n");
        getchar();
        return 1;
    }
    //2)Reads a nmber of devices
    printf("Amount of devices: %d \n",amountOfCP2112(&adapterHandle));
    //3) Initialization of the chip
    if( initAdapter(&adapterHandle, 100000, 100, 0, informationMessage) != 0) {
            puts(informationMessage);
        getchar();
        return 0;
    } else {
        puts(informationMessage);
        puts("Enter number of bytes \n");
        scanf("%d",&numberOfBytes );
        //4) Reading and writing data
          readData(&adapterHandle,textString,0x20,informationMessage,numberOfBytes);
         writeData(&adapterHandle, (char*)bigConstants, 0x20, informationMessage, numberOfBytes);
